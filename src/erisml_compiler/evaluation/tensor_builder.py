...
def build_timeline(
    ...
    *,
    ...
    `ethos_weights` is forwarded to every projection so each timeline
    snapshot uses the same fitted ethos.
    """
    if not ir.events:
        em_outputs = dag.evaluate(ir)
        vector = build_moral_vector_from_em_outputs(em_outputs, dag, ethos_weights=ethos_weights)
