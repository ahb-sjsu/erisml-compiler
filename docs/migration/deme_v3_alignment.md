...
- [x] Phase 6: Strategic layer + decision proofs — done 2026-06-12 (final phase, V3 alignment complete). erisml_backend/v3_phase6.py adds three artifacts on top of Phase 5: (1) `compute_strategic_analysis` runs Shapley attribution + welfare-metric computation against the V3 rank-2 tensor via `erisml.ethics.game_theory.compute_shapley_from_tensor`; surfaced on `ir.strategic_analysis`. (2) `build_decision_proof` constructs a `DecisionProof`-shaped JSON dict with input-facts hash, profile hash, layer outputs, ranked options, forbidden options, moral-vector hash, and a previous_proof_hash chain back to AuditRecord.ir_hash; surfaced on `ir.decision_proof`. (3) `build_coalition_c_axis_slices` makes the c axis on ranks 4-6 REAL — per-coalition slices via grand_only/all_subsets/singletons_only/pairwise enumeration, with non-coalition members zeroed. CLI: `--coalition-mode`. Verified on nazi_attic: Shapley values per stakeholder, real per-coalition variation (c-axis std 0.43) at rank-4 under all_subsets, proof_hash chains correctly. 15 new tests. V3 migration is complete; CompilerIR now carries 6 V3 surface fields (moral_tensor_v3, per_party_verdicts, fairness_metrics, strategic_analysis, decision_proof, schema_version=v0.2).
- [ ] Phase 7: Silicon emit migration
- [ ] Phase 8: I-EIP Monitor migration
- [ ] Phase 9: Docs + JOSS paper
